...
//  public static final RawPageSpace offHeapMetaDataSpace = new RawPageSpace("meta", VMRequest.discontiguous());
...
//      m.remSetLogBuffer = offHeapMetaDataSpace.acquire(1);
//    }
  }
...
//      G1Mutator.newMutatorBarrierActive = false;
      ...
      CardTable.clearAllCardMarks();
      // Release global remsets
      for (int i = 0; i < G1Mutator.globalRemSetLogBuffer.length(); i++) {
        Address buf = G1Mutator.globalRemSetLogBuffer.get(i);
        if (!buf.isZero()) {
          offHeapMetaDataSpace.release(buf);
          G1Mutator.globalRemSetLogBuffer.set(i, Address.zero());
        }
      }
      G1Mutator.globalRemSetLogBufferCursor = 0;
      return;
    }

...
//    boolean result = VM.barriers.objectReferenceTryCompareAndSwap(getForwardingPointer(src), old, tgt, metaDataA, metaDataB, mode);
...
//    return bulkCopy(src, srcOffset, dst, dstOffset, bytes);
    ...
    while (dstCursor.LT(limit)) {
      ObjectReference ref = getForwardingPointer(dstCursor.loadObjectReference());

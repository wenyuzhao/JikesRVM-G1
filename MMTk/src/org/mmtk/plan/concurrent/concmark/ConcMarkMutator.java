...
public class ConcMarkMutator extends ConcurrentMutator {
  ...
  public Allocator getAllocatorFromSpace(Space space) {
    if (space == ConcMark.markRegionSpace) return zgc;
  ...
  protected void checkAndEnqueueReference(ObjectReference ref) {
    ...
    if (barrierActive) {
      if (!ref.isNull()) {
        if      (Space.isInSpace(ConcMark.RC, ref)) ConcMark.markRegionSpace.traceObject(remset, ref);
  ...
  /*
    if (VM.VERIFY_ASSERTIONS) {
      if (!ref.isNull() && !Plan.gcInProgress()) {
        if      (Space.isInSpace(ConcMark.MARK_SWEEP, ref)) VM.assertions._assert(ConcMark.msSpace.isLive(ref));
